# a file for generating .wav files of chords for chroma training
# requires a dir of soundfonts (.sf2) and a dir of midi files (.mid)
...
def to_audio(sf2, midi_file, out_dir, out_type='wav', txt_file=None, append=True):
    ...
    sf2name = os.path.splitext(os.path.basename(sf2))[0]
    if not txt_file:
    ...
    try:
        ...
        if not textfiles or len(textfiles) < len(midifiles):
            for i in range(0, NUM_TRAIN_SAMPLES):
                to_audio(random.choice(sf2files), random.choice(midifiles), out_dir, out_type)
        else:
